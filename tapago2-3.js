...
window.onload = loadHoje();

function loadHoje() {
    mes.value = diaHoje.getMonth() + 1;
    mudarTituloMes();
    loadMes();
    data.value = diaHoje.getDate();
    loadDiaHoje();

 const celulas = document.getElementsByTagName('td');
    const diaAtual = diaHoje.getDate();
    celulas[diaAtual - 1].classList.add('focado');
}


function loadDiaHoje() {
    const tituloVelho = document.querySelector('h2');
    const tituloNovo = document.createElement('h2');
    const options = {weekday: "long", month: "long", day: "numeric"};
    const diaPorExtenso = new Intl.DateTimeFormat ("pt-BR", options).format(diaHoje);
...
function gerarCalendario(year, month) {
    ...
    while (mesEscolhido.getMonth() == mesNumero) {
        ...
        celula.setAttribute('id', 'cell_' + numeroDia); // Assigning individual id to each cell
        tblBody.appendChild(celula);
...
data.addEventListener('change', () => loadDia());
